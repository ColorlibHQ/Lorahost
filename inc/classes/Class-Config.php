...
// Final Class
final class lorahost {

	// Theme Version
	private $lorahost_version = '1.0';
	...
	private function setup() {
		...
		$enqueu = new lorahost_Enqueue();
		$enqueu->scripts = $this->enqueue() ;
		$enqueu->lorahost_scripts_enqueue_init() ;

	}
	// Theme Support
	public function support() {
		// content width
        $GLOBALS['content_width'] = apply_filters( 'lorahost_content_width', 751 );

        
        // text domain for translation.
        load_theme_textdomain( 'lorahost', LORAHOST_DIR_PATH . '/languages' );
		...
		add_image_size( 'lorahost_widget_post_thumb', 80, 80, true );

		// Custom  thumbnails size for next prev thumb
		add_image_size( 'lorahost-np-thumb', 60, 60, true );
        ...
        add_theme_support( 'custom-header', array(
			...
			'flex-height'   => true,
			'height'        => 450,
			'default-image' => get_template_directory_uri() . '/assets/img/page_header.png'
		) );
        ...
        register_nav_menus( array(
            'primary-menu'   => esc_html__( 'Primary Menu', 'lorahost' ),
            'social-menu'   => esc_html__( 'Social Menu', 'lorahost' ),
            'footer-menu'   => esc_html__( 'Footer Menu', 'lorahost' )
        ) );
	...
	private function enqueue() {
		...
		$cssPath = LORAHOST_DIR_CSS_URI;
		$jsPath  = LORAHOST_DIR_JS_URI;
		...
		$scripts = array(
			'style' => array(
				...
				array(
					'handler'		=> 'lorahost-main',
					'file' 			=> $cssPath.'main.css',
					'dependency' 	=> array(),
					'version' 		=> $this->lorahost_version,
				),
				array(
					'handler'		=> 'lorahost-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'lorahost' ) ) {
	...
	private function customizer_init() {
		...
		// Instantiate lorahost theme customizer
		$lorahost_theme_customizer = new lorahost_theme_customizer();
	}
	...
	public function epsilon_quickie() {
			...
			'links' => array(
				array(
					'link_to'   => 'lorahost_options_panel',
	...
	public function set_elementor_flag() {
		update_option( 'lorahost_had_elementor', 'no' );
	}
	...
	public function elementor_desiable_default_style() {
		...
		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'lorahost-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'lorahost_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style() {
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'lorahost-elementor-default-style',  LORAHOST_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->lorahost_version );
		}
	}
	...
	public function enqueue_elementor_notice_script() {
		...
		$had_elementor = get_option( 'lorahost_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ) {
			wp_enqueue_script( 'lorahost-elementor-notice', LORAHOST_DIR_JS_URI.'lorahost-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'lorahost-elementor-notice',
				'lorahostElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'lorahost-elementor-notice-nonce' ),
				)
			);
		}
