...
class repair_newsletter_widget extends WP_Widget {
    ...
    function __construct() {
        ...
        esc_html__( '[ Repair ] Newsletter Form', 'lorahost' ),

        // Widget description
        array( 'description' => esc_html__( 'Add footer newsletter signup form.', 'lorahost' ), )
        );
    ...
    <div id="mc_embed_signup" class="newsletter">
        ...
        <form action="<?php echo esc_url( $actionurl ); ?>" id="mc-embedded-subscribe-form" method="post">
            <div class="form-group">
                <input type="email" class="form-control" name="EMAIL" placeholder="<?php esc_html_e( 'Email address', 'lorahost' ); ?>" required>
                <button class="sideabr_news_submit btn click-btn" type="submit">
                    <span class="ti-arrow-right"></span>
                </button>
            </div>
            <button class="button rounded-0 primary-bg text-white w-100" type="submit"><?php echo esc_html__( 'Subscribe', 'lorahost' ); ?></button>
    ...
    public function form( $instance ) {
        ...
        if ( isset( $instance[ 'title' ] ) ) {
            ...
            $title = esc_html__( 'Newsletter', 'lorahost' );
        }
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'actionurl' ) ); ?>"><?php esc_html_e( 'Action URL:' ,'lorahost'); ?></label>
            <p class="description"><?php esc_html_e( 'Enter here your MailChimp action URL.' ,'lorahost'); ?> </p>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'desc' ) ); ?>"><?php esc_html_e( 'Short Description:' ,'lorahost'); ?></label>
